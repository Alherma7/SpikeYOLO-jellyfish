...
def compute_ap(recall, precision):
    ...
    if method == 'interp':
        ...
        trapz = np.trapz if hasattr(np, 'trapz') else np.trapezoid  # np.trapz removed in numpy>=2.0
...
def ap_per_class(tp,
    ...
    # Find unique classes
    unique_classes, nt = np.unique(target_cls, return_counts=True)  #  返回每个类有多少个target
    nc = unique_classes.shape[0]  # number of classes, number of detections

    #1111
    ap_s=np.zeros((nc, tp.shape[1]))
    ap_m=np.zeros((nc, tp.shape[1]))
    ap_l=np.zeros((nc, tp.shape[1]))
